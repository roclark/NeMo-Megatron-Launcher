...
def main(cfg):
    """Function to download the Slim Pajama dataset files.

    Arguments:
        cfg: main config file.
    """
    data_dir = cfg.get("data_dir")
    slim_pajama_url_train = cfg.get("slim_pajama_url")
    chunks = cfg.get("chunks")
    shards = cfg.get("shards")

    if "-" in chunks:
        min_chunk, max_chunk = chunks.split("-")
    else:
        min_chunk, max_chunk = chunks
    ...
    for chunk in range (int(min_chunk), int(max_chunk) + 1):
        proc_list = []
        shards_to_download = utils.split_list(list(range(shards)), wsize)

        for shard in shards_to_download[wrank]:
            output_file = f"example_train_chunk{chunk}_shard{shard}.jsonl.zst"
            url = os.path.join(slim_pajama_url_train, f"chunk{chunk}", f"example_train_{shard}.jsonl.zst")

            # Test if the URL exists before downloading
            req = requests.head(url)
            if req.status_code == 404:
                # The URL doesn't exist, skip to the next file to prevent
                # downloading an empty file detailing the error
                continue
    ...
    #    url = f"{slim_pajama_url_train}{file_number:02d}.jsonl.zst"
